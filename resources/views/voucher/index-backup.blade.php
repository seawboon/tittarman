                            ...
                            <tbody class="list">
                              @foreach($vouchers as $voucher)
                                <tr>
                                    ...
                                    <td class="align-top">
                                    @if(isset($voucher->owner))
                                    <a href="{{ route('patient.edit', $voucher->owner)}}"><span class="small">{{ $voucher->owner->salutation ?? ''}}</span> {{ $voucher->owner->fullname ?? ''}}</a>
                                    @endif
                                    </td>
                        ...
                        <table class="table align-items-center">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th scope="col" class="sort" data-sort="budget">Code</th>
                                    <th scope="col" class="sort" data-sort="status">Status</th>
                                    <th scope="col" class="sort" data-sort="budget">Transfer to</th>
                                    <th scope="col" class="sort" data-sort="status">Date</th>
                                    <th scope="col" class="sort" data-sort="branch" style="width:40px !important; padding: .75rem 0"></th>
                                </tr>
                            </thead>
                            <tbody class="list">
                              @foreach($transfers as $transfer)
                                <tr>
                                    ...
                                    <td class="budget align-top">
                                      <span class="code-{{$loop->iteration}} mr-2">{{ $transfer->code }}</span>
                                      <button class="copyCode border-0 bg-transparent" data-clipboard-action="copy" data-clipboard-target="span.code-{{$loop->iteration}}"><i class="ni ni-single-copy-04"></i></button>
                                    </td>
                                    ...
                                    <td class="align-top">
                                      <span class="{{ $transfer->state=='enable' ? 'text-success':'text-danger'}}">
                                        {{ $transfer->state }}
                                      </span>

                                    </td>
                                    <td class="budget align-top">
                                      <a href="{{ route('patient.edit', $transfer->patient)}}"><span class="small">{{ $transfer->patient->salutation }}</span> {{ $transfer->patient->fullname }}</a>
                                    </td>
                                    <td colspan="2" class="align-top">
                                        {{ Carbon\Carbon::parse($transfer->transfer_date)->format('d M Y') }}
                                    </td>
                                    {{--<td class="align-top px-0">
                                      @if($transfer->state == 'enable')
                                      <div class="dropdown float-right">
                                          <!--<a class="btn btn-sm btn-info ml-2" href="{{ route('checkin.store', ['patient' => $patient]) }}">{{ __('ttm.checkin')}}</a>-->
